...
def pycondor_submit(batch_name, env_file, sub_dir, out_dir, in_reproc_filename, in_orig_filename, sleep_time = 1, priority = 5):
    ...
    delayed_orig_dir="/data/snoplus3/parkerw/ratSimulations/Nov20_origSFDelay/"
    delayed_reproc_dir="/data/snoplus3/parkerw/ratSimulations/Oct7_fastSFDelay_copy/"

    prompt_list="/data/snoplus3/parkerw/BiPo/dataPromptGTIDs.txt"
    delayed_list="/data/snoplus3/parkerw/BiPo/dataDelayedGTIDs.txt"
    ...
    base_filename = "{0}/condor_base_bipo.mac".format(sub_dir)
    ...
    out_macro_text = raw_text.substitute(sh_sub=env_file, other_commands_sub=other_commands, prompt1=in_reproc_filename, prompt2=in_orig_filename, promptlist=prompt_list, delaylist=delayed_list, delay1=delayed_orig_dir, delay2=delayed_reproc_dir, outfile=outfile)
...
if __name__ == "__main__":
    ...
    parser.add_argument("-r", "--prompt_reproc_dir", type=str,
                        default="prompt_reproc_dir",
                        help="reproc input prompt files directory")
    parser.add_argument("-o", "--prompt_orig_dir", type=str,
                        default="prompt_orig_dir",
                        help="orig input prompt files directory")
    ...
    in_reproc_dir = check_dir(args.prompt_reproc_dir)
    in_orig_dir = check_dir(args.prompt_orig_dir)
