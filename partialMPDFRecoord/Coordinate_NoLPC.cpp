...
void GetPartialPDF(std::string filePathName, std::string material, unsigned int numberOfRuns, double velocity=-999, int pmtBand=0, int numPDFs=0)
{
  ...
  histogram1->Scale(1000000/histogram1->Integral());
  //  histogram2->Scale(100000/histogram2->Integral());
  ...
  std::cout << "pmt_z_range: [" << pmtzmin << ", " << pmtzmax << "]," << std::endl;
  std::cout << "vertex_r_range:  [" << vertexrmin << ", " << vertexrmax << " ]," << std::endl;
...
void FillPartialTimeResidualsPlot(std::string infile, TH1D* histogram, double velocity, int pmtBand)
{
  ...
  for(size_t i = 0; i < dsReader.GetEntryCount(); i++) 
  {
    ...
    for(size_t j = 0; j < calibratedPMTs.GetCount(); j++)
    {
      ...
      if(pmtBand % 4 == 0 && pmtPos.Z() >= boundary + fFillLevelCentre.Z()) //855.48 = 108+747.48 (av offset + fill level)
	histogram->Fill(timeResid);
      if(pmtBand % 4 == 1 && pmtPos.Z() >= 0 + fFillLevelCentre.Z() && pmtPos.Z() < boundary + fFillLevelCentre.Z())
        histogram->Fill(timeResid);
      if(pmtBand % 4 == 2 && pmtPos.Z() >= -boundary + fFillLevelCentre.Z() && pmtPos.Z() < 0 + fFillLevelCentre.Z())
        histogram->Fill(timeResid);
      if(pmtBand % 4 == 3 && pmtPos.Z() < -boundary + fFillLevelCentre.Z())
	histogram->Fill(timeResid);
    }
  }
}
