...
#def write_macro(mac, macname, outfile, velocity, lower_bound, upper_bound):
#    with open(macname, "w") as f:
#        f.write(mac.format(os.path.abspath(outfile), velocity, lower_bound, upper_bound))

def write_macro(mac, macname, outfile, lower_bound, upper_bound):
    with open(macname, "w") as f:
        f.write(mac.format(os.path.abspath(outfile), lower_bound, upper_bound))
...
if __name__ == "__main__":
    ...
    for bound in np.arange(0, 6000, 1000):

        full_dir = args.out_dir + "Dec12_" + str(bound) + "to" + str(bound+1000) + "mm"
        ## check if output and condor directories exist, create if they don't
        out_dir = check_dir(full_dir)
        ...
        write_macro(mac,
                    "{0}{1}.mac".format(mac_dir, base_name),
                    "{0}{1}.root".format(out_dir, base_name),
                    bound,
                    bound+500
        )
        job_id = "{0}".format(base_name)
        batch_id = "rat_{0}".format(base_name)
        pycondor_submit(batch_id,job_id,"{0}{1}.mac".format(mac_dir,base_name),args.run_directory,args.env_file,args.submission_directory,out_dir, sleep_time = 1, priority = 5)

        #for k in np.arange(182, 188.5, 0.5):
        #for k in np.arange(145, 225, 10):
            #for k in np.arange(185, 191.5, 0.5):
            #for i in range(args.no_sims):
            #    j=0.8+i*0.2
            #    j=i
                   #write_macro(mac,
                   # "{0}{1}_{2}.mac".format(mac_dir, base_name, k),
                   # "{0}{1}_{2}.root".format(out_dir, base_name, k),
                   # k,
                   # bound,
                   # bound+500
        # )
        #        job_id = "{0}_{1}".format(base_name,k)
        #batch_id = "rat_{0}".format(base_name)
        #pycondor_submit(batch_id,job_id,"{0}{1}_{2}.mac".format(mac_dir,base_name,k),args.run_directory,args.env_file,args.submission_directory,out_dir, sleep_time = 1, priority = 5)
