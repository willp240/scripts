...
static bool ParseRSP(const char* fname, long long& run, int& sub, int& part) {
  ...
  int n = std::sscanf(fname,
                      "Analysis20_PPOR_r%lld_s%d_p%d.root_reproc.root",
                      &run, &sub, &part);
  if (n == 3) return true;
  n = std::sscanf(fname,
                  "Analysis20_PPOR_r%lld_s%d_p%d.root",
                  &run, &sub, &part);
  return n == 3;
}

void pair_open(const char* reproc_dir =
                 "/data/snoplus3/parkerw/ratSimulations/Oct6_Ana20PPO",
               const char* orig_dir   =
                 "/data/snoplus3/SNOplusData/processing/fullFill/rat-8.0.1/Analysis20_PPOR/ratds",
  ...
  TFile *file = TFile::Open("/data/snoplus3/parkerw/ppodatatree.root", "RECREATE");
