...
void GetScintPDF(std::string material, unsigned int numberOfRuns, double velocity=-999)
{
  ...
  for(unsigned int i = 0; i < numberOfRuns; i++)
    {
      ...
      fileNameStream << "ET1DPartial.root";
  ...
  std::cout << std::endl;
  std::cout << "{" << std::endl;
  std::cout << "type : \"ET1D_Partial_PMTZ0to1.5\"," << std::endl;
  ...
  TFile *outfile = TFile::Open("ET1D_PMTZ0to1.5_PDF.root","RECREATE");
  histogram1->Write("MC");
