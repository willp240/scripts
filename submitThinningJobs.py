...
#    condor_dir = check_dir(condor_directory)
    ...
    for i in range(args.no_sims):
        ...
        #j=i
        write_macro(mac,
                    "{0}{1}_{2}.mac".format(mac_dir, base_name, j),
                    "{0}{1}_{2}.root".format(out_dir, base_name, j),
                    j
        )
        
        job_id = "{0}_{1}".format(base_name,j)
        batch_id = "rat_{0}".format(base_name)
        pycondor_submit(batch_id,job_id,"{0}{1}_{2}.mac".format(mac_dir,base_name,j),args.run_directory,args.env_file,args.submission_directory,out_dir, sleep_time = 1, priority = 5)
