...
def saveTree(fname, outfile):
    ...
    tMin = np.empty((1), dtype="float32")
    posTree.Branch("tMin", tMin, "tMin/F")
    tMax = np.empty((1), dtype="float32")
    posTree.Branch("tMax", tMax, "tMax/F")
    ...
    for ds, run in rat.dsreader(fname):
        ...
        for iev in range(0,ds.GetEVCount()):
            ...
            for i in range(calibratedPMTs.GetCount()) :
                ...
                if tHit < tmin and tHit > 0:
                    tmin = tHit
                if tHit > tmax and tHit > 0:
                    tmax = tHit
