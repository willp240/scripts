...
def write_macro(mac, macname, outfile, thinning_factor, cutoff):
    with open(macname, "w") as f:
        f.write(mac.format(os.path.abspath(outfile), thinning_factor, cutoff))
...
#    condor_dir = check_dir(condor_directory)
    ...
    for k in 12, 30, 50, 100, 250, 500, 1000, 10000:

        for i in range(args.no_sims):
            j=0.8+i*0.2
            #j=i
            write_macro(mac,
                        "{0}{1}_{2}_{3}.mac".format(mac_dir, base_name, j, k),
                        "{0}{1}_{2}_{3}.root".format(out_dir, base_name, j, k),
                    j, k
            )
        
            job_id = "{0}_{1}_{2}".format(base_name,j,k)
            batch_id = "rat_{0}".format(base_name)
            pycondor_submit(batch_id,job_id,"{0}{1}_{2}_{3}.mac".format(mac_dir,base_name,j,k),args.run_directory,args.env_file,args.submission_directory,out_dir, sleep_time = 1, priority = 5)
