...
void photonThinPlotErrLT()
{
  ...
  TFile *outtfile = new TFile("Jul16_Thinning_Poisson30","RECREATE");

  double basePEMean=1, baseNHitsMean=1, basePEError =0;

  for(int i=0; i<55; i++){

    double thinFactor = 0.6 + i*0.08;
    int date = 16;
    if (i % 5 == 0)
      eventFile = Form("/data/snoplus/parkerw/ratSimulations/batch/Jul%d_Thinning_Poisson30/photon_thinning_%.1f.root", date, thinFactor);
    else
      eventFile = Form("/data/snoplus/parkerw/ratSimulations/batch/Jul%d_Thinning_Poisson30/photon_thinning_%.2f.root", date, thinFactor);
    /*else{
      date = 23;
      if(i > 54) date = 24;
      if(i % 5 == 0)
	eventFile = Form("/data/snoplus/parkerw/ratSimulations/batch/Jun%d_Thinning/photon_thinning_%.1f.root", date, thinFactor);
      else
	eventFile = Form("/data/snoplus/parkerw/ratSimulations/batch/Jun%d_Thinning/photon_thinning_%.2f.root", date, thinFactor);
	}*/
    ...
    MCPEhisto->SetBinContent(i+9, (g1->GetParameter(1) - basePEMean) / basePEMean);
    if(g1->GetParameter(1) == basePEMean)
      MCPEhisto->SetBinContent(i+9,0.000001);
    std::cout << (g1->GetParameter(1) - basePEMean) / basePEMean << std::endl;
    MCPEhisto->SetBinError(i+9, GetError( basePEMean, basePEError, g1->GetParameter(1), g1->GetParError(1)) );
...
double GetError( double baseMean, double baseError, double mean, double error) {
  ...
  if( abs(baseMean - mean) < 0.01)
    return 0;
  std::cout << ">> " << abs(baseMean - mean) << " " << baseMean << " " << mean << std::endl;
