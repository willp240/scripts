...
void GetFullPDF(std::string filePathName, std::string material, unsigned int numberOfRuns, double velocity=-999, int numPDFs=0)
{
  ...
  TH1D* histogram1[8];// = new TH1D("histogram1", "", numBins, lowLim-binWidth/2., upLim+binWidth/2.);
  TH1D* histogram2[8];// = new TH1D("histogram2", "", numBins, lowLim-binWidth/2., upLim+binWidth/2.);
  ...
  for(unsigned int i = 0; i < numberOfRuns; i++)
    {
      ...
      fileNameStream << filePathName + "_part" << i+1 << ".root";
  ...
  for(int ihist=0; ihist < numPDFs; ihist++) {
    ...
    std::cout << "distance_range:  [0.00, 40000.00 ]," << std::endl;
    std::cout << "vertex_r_range:  [" << distmin[ihist] << ", " << distmax[ihist] << " ]," << std::endl;
...
void FillPartialTimeResidualsPlot(std::string infile, TH1D* histogram[6], double velocity)
{
  ...
  for(size_t i = 0; i < dsReader.GetEntryCount(); i++) 
  {
    ...
    for(size_t j = 0; j < calibratedPMTs.GetCount(); j++)
    {
      ...
      if(pmtPos.X() > 0 && pmtPos.Y() > 0 && pmtPos.Z() > 0)
	histogram[0]->Fill(timeResid);
      else if (pmtPos.X() < 0 && pmtPos.Y() > 0 && pmtPos.Z() > 0)
	...
	histogram[6]->Fill(timeResid);
      else if (pmtPos.X() > 0 && pmtPos.Y() < 0 && pmtPos.Z() < 0)
	histogram[7]->Fill(timeResid);
      ...
      histogram[3]->Fill(timeResid);*/
      /*	
      if(mcPosition.Mag() < 3000 ) 
	histogram[0]->Fill(timeResid);
      else 
	histogram[1]->Fill(timeResid);
      if(mcPosition.Mag() < 2000 && mcPosition.Mag() > 1000 )
        histogram[1]->Fill(timeResid);
      if(mcPosition.Mag() < 3000 && mcPosition.Mag() > 2000 )
        histogram[2]->Fill(timeResid);
      if(mcPosition.Mag() < 4000 && mcPosition.Mag() > 3000 )
        histogram[3]->Fill(timeResid);
      if(mcPosition.Mag() < 5000 && mcPosition.Mag() > 4000 )
        histogram[4]->Fill(timeResid);
      if(mcPosition.Mag() < 5000 )
      histogram[5]->Fill(timeResid);*/
    }
  }
}
